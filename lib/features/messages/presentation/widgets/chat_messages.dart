...
import 'package:instagram_dharmesh_bloc_demo/features/messages/presentation/widgets/record_view.dart';

import '../../../../core/asset/asset.dart';
import '../../../../core/colors/app_colors.dart';
import '../../../../core/constants/constants.dart';
import '../../../../core/customPackages/audio_recorder/social_media_recoder.dart';
...
import '../bloc/message/cubit/group_chat/message_for_group_chat_cubit.dart';
import '../bloc/message/cubit/message_cubit.dart';
import 'chat_page_component/shared_message.dart';
...
class _ChatMessagesState extends State<ChatMessages>
  ...
  Widget buildSingleChat(BuildContext context) {
    return ValueListenableBuilder(
      ...
      builder: (context, bool reLoadValue, child) => BlocBuilder<MessageBloc, MessageBlocState>(
        builder: (context, state) {
          if (state is MessageBlocInitial) {
            // Initial state, dispatch event to load messages
            BlocProvider.of<MessageBloc>(context).add(LoadMessagesForSingleChat(receiversInfo[0].userId));

            return isThatMobile ? buildCircularProgress() : const ThineLinearProgress();
          } else if (state is MessageBlocLoaded) {
            return buildMessages(context, state.messages);
          } else if (state is MessageBlocError) {
            return Text('Error loading messages: ${state.message}');
          } else {
            // Handle other states if necessary
            return buildCircularProgress(); // Placeholder for loading state
          }
        },
      ),
    );
  ...
  Container buildMessage(
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        border: isThatMe ? null : Border.all(color: AppColors.lowOpacityGrey),
      ),
      clipBehavior: Clip.antiAliasWithSaveLayer,
      padding: !messageInfo.isThatImage
  ...
  Text textMessage(String message, bool isThatMe) {
    TextStyle style = isThatMe
        ? getNormalStyle(color: AppColors.white)
  ...
  Widget pickPhoto(MessageCubit messageCubit) {
    return GestureDetector(
      ...
      child: SvgPicture.asset(
        isThatMobile ? gallery : galleryBold,
